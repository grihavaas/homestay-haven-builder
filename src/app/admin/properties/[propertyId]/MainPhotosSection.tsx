...
import type { UploadDestination } from "./AddPhotosButton";
...
export function MainPhotosSection({
  propertyId,
  tenantId,
  items,
  rooms,
  ...
  const destination: UploadDestination = { mediaType: "hero" };

  return (
    <section className="rounded-lg border border-zinc-200 bg-white p-4">
      <div className="flex items-center justify-between gap-4 mb-3">
        <h3 className="text-base font-semibold text-zinc-900">Main photos</h3>
        <AddPhotosButton
      ...
      <p className="text-xs text-zinc-500 mb-3">These appear in the carousel at the top of your property page.</p>
      {sorted.length === 0 ? (
        <p className="text-sm text-zinc-500 py-4">No main photos yet.</p>
      ) : (
